...
**Status:** Ratified — 2026-07-12 by Laurentiu, **scope: pilot authorization** (Confidence Medium; full adopt-ratification after the calibration-pair pilot re-derives confidence). Drafted 2026-07-11; research closed (taxonomy ratified in owner discussion, census-at-scale via P1, judge design via R4).
...
**Routing-boundary home (owner decision, 2026-07-12):** the shared "algorithm-shaped vs rule/mapping-shaped" routing contract will live **once, in the mine-family core doc** (shipped nexus 1.26.1) — not restated per-proposal. **It is not authored there yet:** the first sibling to graduate-to-spec writes it into the core in that single session; both skills then cite it. On drift, supersede the core, never fork the definition.
...
**Confidence:** Medium — the load-bearing unconfirmed assumption is stage-2/3 efficacy: "best design" has no oracle, and the R4-grounded two-tier judge is research-backed but unproven on this estate. No further research closes this — only the pilot can (re-derive after it runs). Stage 1 and the gate mechanics carry no unconfirmed assumptions (same verification shape as the BR mines).
**Impact:** 7
**Effort:** med
**Date:** 2026-07-11
