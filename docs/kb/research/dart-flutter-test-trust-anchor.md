...
**Status:** current — core verdict HANDS-ON CONFIRMED 2026-06-24 (mutation_test runs on a real Flutter class driving `flutter test`, machine-readable `-f xml`; dart_mutant confirmed not pub-installable). The dart_mutant-vs-`flutter test` detail stays doc-inferred (untestable here — not installable)
...
**Hands-on confirmation (2026-06-24):** `mutation_test` 1.8.0 was installed and run against a real Flutter class (`BuildZplCodeUsecase` in omnishelf_flutter_app), driving `flutter test` via its XML config, producing a machine-readable `-f xml` report — scoring 90% (19/21), the 2 survivors being equivalent log-call mutants. `dart_mutant` was confirmed NOT pub-installable (`dart pub global activate dart_mutant` fails — not a pub package). `flutter test --coverage` was not re-probed for branch coverage. So the core verdict (mutation_test is the practical mechanical gate for Flutter; multi-signal otherwise) is now first-hand validated; the remaining doc-inferred items are below.
