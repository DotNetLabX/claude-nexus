...
**Escalation:** if ANY question is `inferred` or `none`, collect them and message the team lead: "For user: {N} questions need your input — PO couldn't answer from spec," including the full Q&A list so the user sees what was answered and what wasn't. On a **boostable ask** (research-before-asking.md's definition — a user-decision question whose confidence rests on an unconfirmed, expensive, fact-shaped input the user could moot by answering), the primary offer form is the clickable **research option** on the question itself — direct sessions render it via `AskUserQuestion`, relayed escalations carry it as the `Research offer` field (questions-format skill); prose is the fallback only where no clickable surface exists: "I can research {X} first — want me to, or do you already have a direction?" See research-before-asking.md for the full semantics. And for a **fact-shaped unknown** — a fact you can't resolve from current context (not a preference, not something to grep) — research is the **default** before you answer, not an offer; see research-before-asking.md (depth dial, capture-before-surface).
...
- Skip the research offer → instead: always offer to research first — as the clickable research option on a boostable ask (primary form), prose only when no clickable surface exists
