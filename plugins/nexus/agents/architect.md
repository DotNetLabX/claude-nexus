...
- **Attestation drift check (plan / done-check time).** When a class in scope has an **attested golden set** (a C2 attestation record at `docs/kb/golden/{Class}.md`), **plan the test update or flag an M3 re-mine** — you never write the tests yourself, so plan the obligation, don't execute it. Forward conditional — no C2 attestation records exist in this codebase yet; this rule activates the day the first one ships (`docs/specs/adhoc-SddLifecycle/definition/tech-spec.md`).
