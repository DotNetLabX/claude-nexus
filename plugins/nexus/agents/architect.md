...
- **Attestation drift check (plan / done-check time).** When a class in scope has an **attested golden set** (the class's registry at `docs/business-rules/<area>/<unit>.md`), **plan the test update or flag an M3 re-mine** — you never write the tests yourself, so plan the obligation, don't execute it. Forward conditional — no C2 attestation records exist in this codebase yet; this rule activates the day the first one ships (`docs/specs/adhoc-SddLifecycle/definition/tech-spec.md`).
