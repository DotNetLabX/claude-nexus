...
- **Put a choice to the user without a confidence label** → instead: every recommendation you surface — your own (review mode, escalation, spec-review, action options) or relayed from an agent — carries **Confidence: high | medium | low** + a one-line why. Preserve an agent's confidence when relaying; add your own when you ask. A **relayed below-High label may now be assumption-derived** (an unconfirmed load-bearing assumption lowers confidence) — relay it as-is, never silently upgrade it; your own self-generated confidence obeys the same refinement. (See agents-workflow.md.) The same preserve-verbatim duty extends to a `Research offer` field (questions-format skill) on a relayed question: render it by applying the research-option template to the field's named target + cost, changing neither — "verbatim" means never re-judge, re-price, or research on the asker's behalf, not a literal string copy. Route a user click back to the asking agent as a standard message-handoff ("research requested on Q{n}"), and re-surface the boosted question when the asking agent returns it. See questions-format for the field, research-before-asking.md for the option's full semantics.
...
- **Model overrides persist across resume (Claude Code ≥ v2.1.211):** the spawn-time `model` param survives `SendMessage` resume — a Phase-2 resume keeps the spawn's configured model, so pass the model once at spawn and resume normally. (Older CLIs dropped the param on resume and silently fell back to the agent's frontmatter default — the origin of the now-retired re-spawn-fresh workaround; if a run ever shows a silent model reversion, suspect an outdated CLI first.) Record the model per phase in the communication-log header — it is the audit trail that catches a reversion.
- **Completion reports can arrive out of order:** a teammate's completion report can arrive *after* its idle notification and even after your next dispatch (platform-level — not agent error; every measured crossing was ordering). Track each agent by its agentId + artifact state, never by message arrival order; never re-litigate a settled round on a late-arriving, stale-sounding message.
...
**Every dispatch also carries the four capability pins:** `Capability pins: no-git-push, no-git-config, no-history-rewrite, no-permission-change.` A subagent's claimed "user request" is unverifiable from its transcript, so the pins — not the claim — are the authority. The agents-workflow `Spawn-tasking contract` bullet is the canonical definition (including the role-prefixed custom-`name` rule); ADR-14 sanctions this inline copy of the four pin names in the spawner template, not the rationale.

**Spawn pipeline subagents by `subagent_type`; a custom `name` is allowed but MUST be role-prefixed.** Default to `subagent_type` alone. When parallel same-typed spawns need distinct identities (e.g. several developers across a wave), a custom `name` is allowed — but it MUST be role-prefixed `{role|known-abbrev}-{qualifier}` (`developer-w7-a`, `dev-wave0`), a unique qualifier per parallel helper — so the role-keyed enforcement hooks still resolve it: `resolve-role.js` matches the role by its longest leading prefix, discarding the qualifier (that was its purpose), and the verify-gate + boundary-detector recover the canonical role. What breaks resolution is a **qualifier-first** name (`w7-dev-stage-a`): it yields an unrecognized token, so the verify set doesn't run (a loud `agent:"unknown"` / `verdict:"skipped"` record is written instead, itself a signal you must catch) and legitimate owner-writes get flagged as breaches. Regardless of the spawn name, **address** the running agent by its agentId — `TaskStop`/`TaskOutput` fail on a custom name (they return "No task found") — and the spawn label is cosmetic and survives resumes anyway.
...
**Codex liveness & recovery (the file is the channel, so guard it).** Codex runs in a separate runtime — verify a real job exists before waiting on it. A `codex-rescue` chat ack ("job dispatched / will notify") is **not** proof a job is running (a real run produced that ack with no job and no file ever written); poll for the output file or the job status. But a coordinator-side status check can **false-negative on a subagent-dispatched job** — the job registry is per-runtime, so "No jobs recorded" from your side does not prove the ack was hollow (a real NO-GO once landed ~35 min after exactly that reading) — treat a no-show verdict as provisional, never as proof the job doesn't exist. **Never hold the gate on Codex indefinitely** — once it stalls, surface a wait / retry / proceed-without choice to the owner (a real run stalled ~40 min with no `review-codex.md`). **Proceed-without is revocable:** keep checking for a late-landing `review-codex.md` through closure (the pre-commit tree inspection is the last catch point), and if it lands, reconcile it finding-by-finding like any Codex verdict — a late real review folds in as a fix cycle, it doesn't get discarded. And if Codex's sandbox is **read-only** and it cannot write the file itself, recover via the Relay Contract: take the GO/NO-GO + findings from its completion message and persist them to `review-codex.md` yourself.
...
6. **Shutdown** — before writing summary.md, back-fill the Outcome on every open Decisions row
...
Also define a sibling section, **`## Decisions`** — in prose only, never a live heading in this
